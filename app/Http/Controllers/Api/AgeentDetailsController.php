...
class AgeentDetailsController extends Controller
{
    ...
    public function MultiOrderAcceptold(Request $request)
    {
        $authorization = $request->header('Authorization');
        $tokenWithoutBearer = str_replace("Bearer ", "", $authorization);
        $accessToken = Agent::where('token', $tokenWithoutBearer)->first();
        ...
        if ($accessToken) {
            ...
            foreach ($partyIds as $key => $partyId) {
                ...
                return $accept->weight = $weights[$key];
                $accept->status = 1;
                $accept->type = "Debit";
                $accept->save();
                $data[] = $accept;
    ...
    public function MultiOrderAccept(Request $request)
    {
        ...
        if ($accessToken) {
            ...
            foreach ($partyIds as $key => $partyId) {
                ...
                if ($accept->wallet_type == $request->wallet_type && $request->wallet_type == "1") {
                    if ($accept->agent_id && $accept->party_id) {
                        $paymentRecords = AdvancePayment::where('agent_id', $accept->agent_id)
                            ->where('party_id', $accept->party_id)
                            ->where('status', '=', '0')
                            ->latest()
                            ->get();

                        foreach ($paymentRecords as $paymentRecord) {
                            $paymentRecord111 = AdvancePayment::where('agent_id', $accept->agent_id)
                                ->where('party_id', $accept->party_id)
                                ->latest()
                                ->first();
                            if ($paymentRecord111) {
                                ...
                                if ($remainingWallet > 0) {
                                    ...
                                    $accept->remaining_amount = $accept->amount - $acceptAmount;
                                    ...
                                    if ($accept->payment == "Received") {
                                        ...
                                        $history->remaining_wallet = $remainingWallet;
                                        $accept->payment = "Credit";
                                        $history->check = "Debit";
                                    }
                                    $history->payment = "Received";
                                    $history->status = $remainingWallet == 0 ? "1" : "0";
                    ...
                    $newwww->date = $accept->date;
                    $newwww->payment = $accept->payment;
                    if ($request->payment == "Received") {
                        $newwww->amount = $accept->amount;
                    } else {
                        $newwww->amount = "0";
                    }
                    $newwww->wallet = "0";
                    $newwww->remaining_wallet = "0";
                    $newwww->party_id = $accept->party_id;
                    $newwww->reason = $accept->reason;
                    $newwww->status = "1";
                    $newwww->type = "1";
                    $newwww->check = "Credit";
                    $newwww->save();
