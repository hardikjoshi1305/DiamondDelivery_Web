...
class SoldWise implements FromView, WithHeadings,WithEvents,ShouldAutoSize
{
    protected $fromDate;
    protected $toDate;
    protected $agentId;
    ...
    public function view(): View
    {
        ...
        ->where(function ($sold1) {
            ...
            if ($this->agentId) {
               $sold1->where('tbl_diamond_list.agent', $this->agentId);
            }
            if ($this->partyId) {
                $sold1->where('tbl_diamond_list.client', $this->partyId);
            }
            if($this->fromDate && $this->toDate) {
              $sold1->whereBetween(DB::raw('DATE(tbl_diamond_list.created_at)'),[$this->fromDate, $this->toDate]);
            }
            $agent = $sold1->orderBy('tbl_diamond_list.agent', 'asc')
            ->get();
